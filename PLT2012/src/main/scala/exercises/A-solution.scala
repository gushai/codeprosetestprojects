package org.codeprose.plt2012.exercises

object execA_solution {
...
def sumPattern(list : List[Int]) : Int = list match {
  case Nil     => 0
  case x :: xs => x + sumPattern(xs)
}
...
def printPattern(list : List[Int]) : String = list match {
  case Nil     => "Nil"
  case x :: xs => x + " :: " + printPattern(xs)
}
