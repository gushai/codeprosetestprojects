...
def substOne(e1: Exp, x: Symbol, e2: Exp) : Exp = e1 match {
  case Num(n) => e1
  case Add(l,r) => Add(substOne(l,x,e2), substOne(r,x,e2))
  case Id(y) => if (x == y) e2 else Id(y)
  case App(f,a) => App(substOne(f,x,e2),substOne(a,x,e2))
  case Fun(param,body) => 
    if (param == x) e1 else Fun(param, substOne(body, x, e2))
}

/* Let's try whether subst produces reasonable results. */
assert( substOne(Add(5,'x), 'x, 7) == Add(5, 7))
assert( substOne(Add(5,'x), 'y, 7) == Add(5,'x))
assert( substOne(Fun('x, Add('x,'y)), 'x, 7) == Fun('x, Add('x,'y)))
